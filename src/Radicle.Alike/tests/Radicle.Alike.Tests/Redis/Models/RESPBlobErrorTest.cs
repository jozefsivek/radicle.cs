...
public class RESPBlobErrorTest
{
    [Fact]
    public void Constructor_NullStringValue_Throws()
    {
        Assert.Throws<ArgumentNullException>(() => new RESPBlobError((string)null!));
    }

    [Fact]
    public void Constructor_NullValue_Throws()
    {
        Assert.Throws<ArgumentNullException>(() => new RESPBlobError((IEnumerable<byte>)null!));
    }

    [Theory]
    [InlineData("")]
    [InlineData("SYNTAX The quick brown fox jumps over the lazy dog")]
    public void Equals_Equal_ReturnsTrue(string str)
    {
        RESPBlobError one = new(str);
        RESPBlobError other = new(str);
    ...
    {
        RESPBlobError one = new(value);
        RESPBlobError other = new(value);
    ...
    {
        RESPBlobError one = new(first);
        RESPBlobError other = new(second);
    ...
    {
        RESPBlobError one = new(string.Empty)
        {
            Attribs = new RESPAttributeValue(new Dictionary<RESPValue, RESPValue>()
            {
                { new RESPSimpleString("foo"), new RESPSimpleString("bar") },
        ...
        RESPBlobError other = new(string.Empty)
        {
            Attribs = new RESPAttributeValue(new Dictionary<RESPValue, RESPValue>()
            {
                { new RESPSimpleString("foo"), new RESPSimpleString("Bar") },
