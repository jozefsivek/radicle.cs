...
public class RESPBigNumberTest
{
    [Theory]
    [InlineData(0)]
    ...
    {
        RESPBigNumber one = new(number);
        RESPBigNumber other = new(number);
    ...
    [Fact]
    public void Equals_EqualLarge_ReturnsTrue()
    {
        RESPBigNumber one = new(new BigInteger(long.MaxValue) + new BigInteger(long.MaxValue));
        RESPBigNumber other = new(new BigInteger(long.MaxValue) + new BigInteger(long.MaxValue));
    ...
    {
        RESPBigNumber one = new(BigInteger.Zero);
        RESPBigNumber other = new(number);
    ...
    public void Equals_AttribsDifferent_ReturnsFalse()
    {
        ...
        RESPBigNumber other = new(BigInteger.Zero)
        {
            Attribs = new RESPAttributeValue(new Dictionary<RESPValue, RESPValue>()
            {
                { new RESPSimpleString("foo"), new RESPSimpleString("Bar") },
